...
var yob = new Array(1983,2005,1994,2001);
...
function printFullAge(years) {
    ...
    while(i < years.length) {
        myArr[i] = 2017-years[i];
        i++;
    }

    for (j = 0; j < myArr.length; j++) {
        if (myArr[j]>=18) {
            //console.log('Person '+j+ ' is full age.');
            fullAge[j] = true;
        } else {
            //console.log('Person '+j+ ' is less than 18 years old.');
            fullAge[j] = false;
        }
    console.log(fullAge[j]);
    }
}
